...
exports.login = function(players,pool,socket,email,password,callback) {
	var validator = new Validator();
	validator.check(email, "Please enter a valid email").notNull().isEmail();
	validator.check(password, "Please enter an alphanumeric password").notNull().isAlphanumeric();
	...
	if( errors.length > 0 ) {
		...
		pool.getConnection(function(err, connection) {
			if(err) {
				...
				connection.query( 'SELECT pID, passwd FROM Player WHERE email=?',[email], function(err,result) {
					if(err) {
						console.log(err);
					}
					if( result.length == 0 ) {
...
exports.register = function(pool,socket,email,password,callback) {
	var validator = new Validator();
	validator.check(email, "Please enter a valid email").notNull().isEmail();
	validator.check(password, "Please enter an alphanumeric password").notNull().isAlphanumeric();
	var errors = validator.getErrors();
			...
			if(err) {
				...
				connection.query( 'SELECT pID FROM Player WHERE email=?',[email], function(err,result) {
					if( result.length > 0 ) {
						...
						var post = {email: email, passwd: password, gameID: 1 };
