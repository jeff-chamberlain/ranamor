			...
			for( var pID in players ) {
				if( players.hasOwnProperty(pID) ) {
					if( pID == id ) {
						...
						var otherFrog = players[pID].frog;
						frogs[otherFrog] ++;
						console.log( "Frog " + otherFrog + "has a count of " + frogs[otherFrog]);
					}
				}
			}
...
exports.pullInfo = function(pool,id,callback) {
	pool.getConnection(function(err, connection) {
		if(err) {
			...
			connection.query('SELECT Frog.fID, commonName, scientificName, geographicInfo, conservationInfo, FImg.fImg FROM Frog INNER JOIN FImg ON FImg.fID = Frog.fID WHERE Frog.active = 1 AND Frog.fID = ?',[id],function(err,result) {
				if(err) {
					console.log(err);
				}
				connection.release();
